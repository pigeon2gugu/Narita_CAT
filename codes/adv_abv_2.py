...
dvdx = np.zeros((12,41,61))
dudy = np.zeros((12,41,61))
...
for i in range(12):
	for j in range(len(lat)):
        	for k in range(1,len(lon)-1):
               		dvdx[i,j,k] = (nv[i,j,k+1]-nv[i,j,k-1])/(2*dx[j])
...
abv = np.zeros((12,41,61))
dabvdx = np.zeros((12,41,61))
dabvdy = np.zeros((12,41,61))
adv_abv = np.zeros((12,41,61))
...
for i in range(12):
        for j in range(len(lat)):
                for k in range(1,len(lon)-1):
                        dabvdx[i,j,k] = (abv[i,j,k+1]-abv[i,j,k-1])/(2*dx[j])

for i in range(12):
        for j in range(1,len(lat)-1):
                for k in range(len(lon)):
                        dabvdy[i,j,k] = (abv[i,j+1,k]-abv[i,j-1,k])/(-2*dy)

adv_abv = -((nu*dabvdx)+(nv*dabvdy))

adv_abv = (adv_abv)*(3600.)*(100000.)
adv_abv = ndimage.gaussian_filter(adv_abv, sigma=1, order=0)
...
for i in range(12):
	...
	clevs1= np.arange(np.min(ngph),np.max(gph)+60,60)
...
fig = plt.savefig('./figures/advection_abv_22')
