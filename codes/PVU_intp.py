...
PVUT = ndimage.gaussian_filter(PVUT, sigma=3, order=0)
...
for i in range(3):
	...
	clevs2 = np.arange(290,380,10)
	cmap = matplotlib.colors.ListedColormap(['purple', 'darkviolet','violet', 'navy', 'blue', 'royalblue', 'deepskyblue','darkgreen','lime','greenyellow','yellow','gold','orange','orangered','red','darkred'])
	clevs1 = np.arange(290,375,5)
	PVUT[ PVUT > max(clevs1) ] = max(clevs1)
	PVUT[ PVUT < min(clevs1) ] = min(clevs1)
	cs=m.contourf(x,y,PVUT[i,:,:], clevs1, cmap=cmap)
	cs1 = m.contour(x,y,PVUT[i,:,:],clevs2, colors='white', linewidths = 1, linestyles='solid')
        cbar=m.colorbar(cs)
	plt.clabel(cs1,fmt='%3.0f',colors='white', fontsize = 7)
        cbar.ax.set_yticklabels(['<290','300','310','320','330','340','350','360','>370'])
	cbar.set_label('Potential temperature $(K)$', fontsize = 7)

plt.subplots_adjust(wspace = 0.42, hspace = 0.30)
fig = plt.savefig('./figures/PVU_temperature')
