...
f = np.zeros((12,721,1440))
vor = np.zeros((12,721,1440))
vort_adv = np.zeros((12,721,1440))

for i in range(12):
	for j in range(len(lat)):
		for k in range(len(lon)):
			f[i,j,k] = ((2.)*(2.*np.pi/86400.)*(np.sin(lat[j]*np.pi/180.)))

for i in range(12):
	vor[i,:,:] = mpcalc.vorticity(u[i,:,:], v[i,:,:], dx, dy, dim_order='yx')

avor = vor + f
avor = ndimage.gaussian_filter(avor, sigma=3, order=0) * units('1/s')

for i in range(12):
	vort_adv[i,:,:] = mpcalc.advection(avor[i,:,:], [u[i,:,:], v[i,:,:]], (dx, dy), dim_order='yx')

vort_adv = vort_adv *(3600)*(100000)
...
for i in range(12):
	...
	cmap = mpl.cm.bwr
	colormesh = m.pcolormesh(x, y, vort_adv[i,:,:], vmin = -5, vmax = 5, cmap=cmap)
...
plt.show()
#fig = plt.savefig('./figures/advection_abv2')
