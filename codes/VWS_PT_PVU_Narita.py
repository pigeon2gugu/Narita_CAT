...
PV = np.zeros((11,32,721,3))

for i in range(11):
	for l in range(1,31):
		for j in range(len(lat)):
			PV[i,l,j,1] = (-9.806)*((PT[i,l-1,j,1]-PT[i,l+1,j,1])/(float(lv[l-1])-float(lv[l+1])))*((dvdx[i,l,j,1]-dudy[i,l,j,1])+((2.)*(2.*np.pi/86400.)*(np.sin(lat[j]*np.pi/180.))))
...
for i in range(11):
	...
	cmap = mpl.cm.Blues
	x, y = np.meshgrid(lv[1:31], lat)
	clevs1= np.arange(-5,10,5)
	clevs2 = np.arange(150,450,10)
	clevs3 = np.arange(0,3.2+0.2,0.2)
	VWS[ VWS > max(clevs3) ] = max(clevs3)
	plt.plot(lat[505],lv[13], color='r', marker='*')
	cs = plt.contour(y, x, PT[i,:,1:31,1], clevs2, alpha = 0.7 , colors='black', linewidths=0.7)
	#cs1 = plt.contour(y, x, PVU[i,:,1:31,1], clevs1, alpha = 0.7 ,colors='black', linewidths=1.3)
	cs2 = plt.contourf(y, x, VWS[i,:,1:31,1], clevs3,  cmap=cmap)
	plt.ylabel('height $(hPa)$',size=10)
	plt.xlabel('latitude', size=10)
	cbar=plt.colorbar(cs2)
	#plt.clabel(cs1, fmt='%3.0f', colors = 'black')
	plt.clabel(cs, fmt='%3.0f', colors = 'black')
	cbar.set_label('vertical windshear $(10^-2/s)$', fontsize = 7)
	plt.gca().invert_yaxis()
plt.subplots_adjust(wspace = 0.40, hspace = 0.30)
fig = plt.savefig('./figures/VWS_PT_PVU_Narita_20181028_29_30')
