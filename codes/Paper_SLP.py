...
for i in range(3):
	...
	m.plot(x1, y1, color='r', marker='*')
        ...
        colormesh = m.pcolormesh(x, y, SLP[i,:,:], vmin = 996, vmax = 1032, cmap=cmap)
        cbar=m.colorbar(colormesh)
        cbar.set_label('Sea level pressure $(hPa)$', fontsize = 7)
	#cs=m.contour(x,y,SLP[n,:,:], clevs.astype(int), colors='red', linewidths = 0.5, linestyles='solid')
	#plt.clabel(cs,fmt='%3.0f',colors='black')
	n = n+4

plt.subplots_adjust(wspace = 0.42, hspace = 0.30)
fig = plt.savefig('./figures/Paper_SLP')
