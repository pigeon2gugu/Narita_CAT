...
for i in range(11):
	...
	clevs1= np.arange(int(4720),int(5960+60),60)
        clevs2= np.arange(6,36,4)
	gph500[i,:,:][ gph500[i,:,:] > max(clevs1) ] = max(clevs1)
	gph500[i,:,:][ gph500[i,:,:] < min(clevs1) ] = min(clevs1)
	vo[i,:,:][ vo[i,:,:] > max(clevs2) ] = max(clevs2)
	m.plot(x1, y1, color='r', marker='*')
	cmap = mpl.cm.Blues
	cs=m.contour(x,y,gph500[i,:,:],clevs1,colors='red', linewidths = 0.3, linestyles='solid')
	cs2=m.contourf(x,y,vo[i,:,:],clevs2, cmap = cmap)
	cbar=m.colorbar(cs2)
...
fig = plt.savefig('./figures/Absolute vorticity_20181028_29_30')
