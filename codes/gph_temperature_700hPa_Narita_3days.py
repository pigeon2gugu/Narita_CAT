...
for i  in range(3):
        for j in range(4):
                        if dd[i] == '30' and tt[j] == '18':
                                ...
                                gph850[k,:,:] = Dataset('data/2018-10-'+dd[i]+'_'+tt[j]+':00:00.nc').variables['Z_GDS0_ISBL'][20,:,:]

				t[k,:,:] = Dataset('data/2018-10-'+dd[i]+'_'+tt[j]+':00:00.nc').variables['T_GDS0_ISBL'][20,:,:]
				k = k+1
...
for i in range(11):
	...
	clevs1= np.arange(int(round(np.min(t[i,100:301,400:641]))),int(round(np.max(t[i,100:301,400:641])))+10.,10.)
        clevs2= np.arange(int(round(np.min(gph700[i,100:301,400:641]))),int(round(np.max(gph700[i,100:301,400:641])))+60.,60.)
	m.plot(x1, y1, color='r', marker='*')
	cs=m.contour(x,y,t[i,:,:],clevs1,colors='red', linewidths = 0.6, linestyles='dashed')
	cs2=m.contour(x,y,gph700[i,:,:],clevs2,colors='blue', linewidths = 0.6,linestyles='solid')
	plt.clabel(cs, colors='red')
	plt.clabel(cs2, colors='blue')

plt.subplots_adjust(wspace = 0.1, hspace = 0.3)

fig = plt.savefig('./figures/GPH_T_700hPa_Narita_201828_29_30')
